        ...
        var lib;
			...
			,	drag:
				...
				,	make: 	function()	//make the draggable node
					...
					{
						lib.drag.copy.setAttribute(lib.name.drag,'active');
					}
					{
			...
			,	drop:
				{	make: 	function()	//make the droppable node
					{	
					{
						lib.drop.node = lib.drag.node;
						lib.drop.node.setAttribute(lib.name.hold,'active');
					}
					{
					//	lib.drop.node = document.createElement(lib.drag.node.tagName);
					...
					//	lib.drop.node['style']['width']  = (lib.drag.node).offsetWidth  + ('px');
					//	lib.drop.node['style']['height'] = (lib.drag.node).offsetHeight + ('px');
						return true;
					}
                ,   move:   function()  //move the droppable node
                    {   for(var j in lib.drag.type)  
                            each.ElementBy[lib.cfgs.mode]
				...
				,	done: 	function()	//drop the droppable node
					...
					{
						lib.drop.node.removeAttribute('h-hold');
					}	
